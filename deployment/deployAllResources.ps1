...
function Read-CliVersion {
    ...
    if ($min_version -gt $cli_version) {
        ...
        Write-Host
        Write-Host "Your are using supported AzureCLI version ${cli_version}"

        return $true
    }
}

function Read-PsVersion {
    param (
        [version]$min_PsVersion = "5.0.0"
    )
    
    [version]$cur_psVersion = $PSVersionTable.PSVersion

    Write-Host "Verifying PowerShell extension version..."

    if ($cur_psVersion -lt $min_PsVersion) {
        Write-Host
        Write-Host "Your current PowerShell version is ${cur_psVersion}. Please install >=5.0.0 version."
        Write-Host "Do you want to install the newest Powershell version?"
        Write-Host

        $option = Get-InputSelection `
                    -options @(0,1)
                    -text "Please choose [1]yes or [0]no:"

        if ( $option == 0 ) {
            Write-Host
            Write-Host "No installing newest version..."
            return $false
        }
        elseif ( $option == 1 ) {
            Write-Host
            Write-Host "Installing newest powershell version..."
            winget search Microsoft.PowerShell
            winget install --id Microsoft.Powershell --source winget
            winget install --id Microsoft.Powershell.Preview --source winget

            $cur_psVersion = $PSVersionTable.PSVersion

            Write-Host "Newest Powershell version ${cur_version} is installed."

            if ($cur_version -lt $min_version) {
                Write-Host
                Write-Host "Something wrong happed while installing Powershell. Ending script."
                return $false
            }
            else {
                return $true
        ...
        Write-Host
        Write-Host "Your current PowerShell Version is ${cur_psVersion}. The version is supported."
...
function Read-CliExtensionVersion {
    ...
    elseif ($min_version -le $extension_version) {
        Write-Host
        Write-Host "Your current version of the extension '${name}' is '${extension_version}'."
...
function New-Deployment() {
    ...
    $ps_valid = Read-PsVersion -min_PsVersion "5.0.0"
    if (!$ps_valid) {
        return $null
    }
    ...
    Write-Host -ForegroundColor Yellow ($script:service_ppal | ConvertTo-Json)
    ...
    $template = Join-Path $root_path "deployment" "azure-deploy.bicep"
    $parameters = Join-Path $root_path "deployment" "azuredeploy.parameters.json"
    $deployment_id = "$($script:project_name)-$($script:env_hash)"
    $deploymentLog = Join-Path $root_path "ErrorLog" "deploymentLog.json"
    ...
    $template_parameters = @{
        ...
        "repoOrgName"              = @{ "value" = "Azure-Samples" }                             # "duochh"
        "repoName"                 = @{ "value" = "azure-digital-twins-unreal-integration" }    # "IoTFloor1"
        "repoBranchName"           = @{ "value" = $(git rev-parse --abbrev-ref HEAD) }          #"value" = "main"
    }
    ...
    $script:deployment_output = New-AzResourceGroupDeployment `
         -ResourceGroupName $script:resource_group_name `
         -Name $deployment_id `
         -Mode Incremental `
         -TemplateFile $template `
         -TemplateParameterFile $parameters

    Write-Host -ForegroundColor Yellow ($script:deployment_output | ConvertTo-Json)
    Set-Content -Path $deploymentLog -Value ($script:deployment_output | ConvertTo-Json)

    if ($script:deployment_output) {
        Write-Host
        Write-Host "Something went wrong. Showing resources were created."
        Write-Host
        Get-AzResource -ResourceGroupName $script:resource_group_name | Format-Table

        Write-Host
        Write-Host "Deploying process fail. Cleaning up some features."
        Remove-AzResourceGroup -Name $script:resource_group_name

        Write-Host "Removing service principal associated with the app registration"
        Remove-AzADServicePrincipal -DisplayName $script:appRegName

        Write-Host
        Write-Host "Removing app registration '$($script:appRegName)' in Azure Active Directory"
        Remove-AzADApplication -DisplayName $script:appRegName

        Write-Host
        Write-Host "Cleaning up done."

        throw "Something went wrong with the resource group deployment. Ending script."        
    }
    elseif ($script:deployment_output.ProvisioningState -ne "Succeeded") {
        Write-Host
        Write-Host "Something went wrong. Showing resources were created."
        Write-Host
        Get-AzResource -ResourceGroupName $script:resource_group_name | Format-Table
        ...
        Write-Host
        Write-Host "Deploy successfully. Showing resources were created."
        Get-AzResource -ResourceGroupName $script:resource_group_name | Format-Table
    }
